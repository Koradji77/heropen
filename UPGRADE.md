...
heropen add --agent xiaokai --content "..."

# ❌ 错误（会报错）
heropen --agent shishi recall --last 10
```
...
v1.7.22 修复了之前 `shishi`、`xiaokai` 等 agent 被硬编码映射到 `_shared.db` 的问题。
升级后调用 `init_db()` 时会**自动执行一次迁移**：
1. 读取 `_shared.db` 中属于 `shishi` / `xiaokai` 的记录
2. 写入各自独立的 `shishi.db` / `xiaokai.db`
3. 对 `_shared.db` 创建备份 `_shared.db.bak`
4. 迁移过程自动去重（按内容前 100 字判断）

如果你在升级**之前**已经手动迁移过数据（如从 `_shared.db` 导出再导入 `shishi.db`），
